# HyperFetch - Edge Extension

Accelerate large file downloads (>100MB) using parallel threads with this Edge extension.
...
```bash
cd native-host
chmod +x native_host.py install-host-edge.sh

# Get extension ID from edge://extensions, then register host
./install-host-edge.sh YOUR_EXTENSION_ID
```
...
- Verify host manifest exists at `~/.config/microsoft-edge/NativeMessagingHosts/com.hyperfetch.host.json`
...
- Open extension service worker logs from `edge://extensions` for details
...
✅ Tokens stored securely in Edge's encrypted storage
...
`~/.config/microsoft-edge/NativeMessagingHosts/com.hyperfetch.host.json`
...
A: No. Native Messaging host must run on the same machine as Edge.
