...
// Firefox does not accept the Chrome-only 'extraHeaders' webRequest extraInfoSpec value (it
// already exposes Cookie/Referer in onBeforeSendHeaders) and has no downloads.onDeterminingFilename
// event, so we branch on this flag where the two engines diverge.
...
function shouldInterceptDownload(filename, totalBytes, minFileSizeBytes) {
  ...
  // Firefox reports totalBytes as -1 (unknown) at onCreated, whereas Chrome reports 0. Treat any
  // non-positive value as "unknown size" so a large-extension file still qualifies on both.
...
function shouldInterceptDownloadItem(downloadItem) {
  ...
  const totalBytes = downloadItem.totalBytes > 0 ? downloadItem.totalBytes : 0;
...
function setupArtifactoryTokenCapture() {
  ...
  chrome.webRequest.onBeforeSendHeaders.addListener(
    ...
    { urls: ['<all_urls>'] },
    // 'extraHeaders' is Chrome-only; Firefox already surfaces Cookie/Referer and throws if it is passed.
...
function setupListeners() {
  ...
  // downloads.onDeterminingFilename is Chrome-only; on Firefox interception happens purely via
  // downloads.onCreated (below), so skip registering this listener there.
  if (!chrome.downloads.onDeterminingFilename) {
    console.log('[MTD] downloads.onDeterminingFilename unavailable (Firefox) — using onCreated only');
  }
  ...
  // Firefox has no downloads.onDeterminingFilename, and downloads.onCreated frequently reports
  // totalBytes as -1 (unknown) because the response headers have not arrived yet. Watch
  // downloads.onChanged so that once the real size or final filename becomes known we can still
  // intercept a large download — provided it is caught early, before much has been written.
  if (IS_FIREFOX && chrome.downloads.onChanged) {
    try {
      chrome.downloads.onChanged.addListener((delta) => {
        if (!delta || typeof delta.id !== 'number') return;
        // React only when size or filename just became known.
        if (!delta.totalBytes && !delta.filename) return;
        if (interceptedDownloads.has(delta.id) || earlyInterceptedIds.has(delta.id)) return;
        if (nativeBypassDownloadIds.has(delta.id)) return;

        Promise.resolve(chrome.downloads.search({ id: delta.id }))
          .then((items) => {
            const item = items && items[0];
            if (!item || item.state !== 'in_progress') return;
            if (interceptedDownloads.has(item.id)) return;
            // Don't cancel a download that already fetched a meaningful chunk — restarting would
            // waste the transferred bytes and be jarring. Only intercept while still near the start.
            const received = item.bytesReceived || 0;
            if (received > 4 * 1024 * 1024) return;
            if (shouldInterceptDownloadItem(item)) {
              console.log('[MTD] Intercepting via onChanged (size/filename now known):',
                item.filename || item.url);
              interceptBrowserDownload(item);
            }
          })
          .catch((err) => console.warn('[MTD] onChanged search failed:', err));
      });
      console.log('[MTD] Download onChanged listener added (Firefox size-based interception)');
    } catch (e) {
      console.error('[MTD] Failed to add onChanged listener:', e);
    }
  }
